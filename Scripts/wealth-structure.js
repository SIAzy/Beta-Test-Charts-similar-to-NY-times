...
function draw_flow_ws_inf(d, element, num_quantile, model_name, parent = "all") { //, qScale_domain, black_ratio_scale, wealth_scale, parent = "all") {
	...
	wealth_scale = get_wealth_scale(d, model_name); // for each group of race and starting quintile, calculates the cumulative proportion that end up in the other destination quintile (4 values, before 100%)
	black_ratio_scale = get_black_ratio_quantile(d, model_name); // black ratio scale seems to be the propportion of black in each parent quintile
	qScale_domain = get_qscale(d, model_name);
	prob_pquantile = get_prob_pquintiles( Object.values(black_ratio_scale) ); //seems like object values is transpose (e.g. 1 record with 5 values each, because 5 record with 1 values each)
	prob_quantile = get_prob_quantiles(d, model_name);
...
function get_black_ratio_quantile(data, model_name){     // this returns the propotion of blacks within each original quintile
	const add = (a, b) => (a + b);
	diction = d3.nest()
			.key( x => x.origin)
			//.key( y => y.race)
