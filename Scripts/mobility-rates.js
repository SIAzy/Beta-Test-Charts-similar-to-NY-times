...
function draw_flow_mr(d, element, num_quantile, model_name, parent) { // black_ratio_scale, wealth_scale, parent) {
	...
	// keep only array item 6 to 10
	quantile_labels.splice(0, 6);
...
if (reglyanshe==1 ) {   // Chinese and Malay
	...
	svg_destination_mr.append('g')
		...
		.attr('class', 'prob-frequency header black-probability')
		.text('Probability for Chinese children');
} else if (reglyanshe==3) {   // Chinese and Others
	svg_destination_mr.append('g')
		.attr('class', 'label-prob-mr label-header')
		.attr('transform', 'translate(0, 0)')
		.append('text')
		.attr('class', 'prob-frequency header white-probability')
		.text('Probability for Other children');
	svg_destination_mr.append('g')
		.attr('class', 'label-prob-mr label-header')
		.attr('transform', 'translate('+ stats_width/3 +', 0)')
		.append('text')
		.attr('class', 'prob-frequency header black-probability')
		.text('Probability for Chinese children');
} else if (reglyanshe==4) {   // Malay and Indian
	svg_destination_mr.append('g')
		.attr('class', 'label-prob-mr label-header')
		.attr('transform', 'translate(0, 0)')
		.append('text')
	...
	svg_destination_mr.append('g')
		...
		.attr('class', 'prob-frequency header black-probability')
		.text('Probability for Malay children');
} else if (reglyanshe==5) {   // Malay and Other
	svg_destination_mr.append('g')
		.attr('class', 'label-prob-mr label-header')
		.attr('transform', 'translate(0, 0)')
		.append('text')
	...
	svg_destination_mr.append('g')
		...
		.attr('class', 'prob-frequency header black-probability')
		.text('Probability for Malay children');
} else if (reglyanshe==6) {   // Indian and Other
	svg_destination_mr.append('g')
		.attr('class', 'label-prob-mr label-header')
		.attr('transform', 'translate(0, 0)')
		.append('text')
...
if  (reglyanshe==1) {  // chinese and malay
	...
	var drawPoints = regl({
		...
		frag: `
			...
			void main() {
				vec4 black = vec4(0.37, 0.42, 0.82, 0.85);
				vec4 white = vec4(0.282, 0.067, 0.314, 0.85);
				gl_FragColor = c == 1.0 ? black : white;
			}`,
...
} else if (reglyanshe==3) { // chinese and other
	var drawPoints = regl({
		vert: `
			precision mediump float;
			attribute float speed, x, y0, y1, dy;
		...
		frag: `
			...
			void main() {
				vec4 black = vec4(0.37, 0.42, 0.82, 0.85);
				vec4 white = vec4(0.259, 0.89, 0.608, 0.85);
				gl_FragColor = c == 1.0 ? black : white;
			}`,
	...
	var drawPoints = regl({
		...
		frag: `
			...
			varying float c;
			void main() {
				vec4 black = vec4(1.00, 0.51, 0.37, 0.85);
				vec4 white = vec4(0.282, 0.067, 0.314, 0.85);
				gl_FragColor = c == 1.0 ? black : white;
			}`,
	...
	var drawPoints = regl({
		...
		frag: `
			...
			varying float c;
			void main() {
				vec4 black = vec4(0.282, 0.067, 0.314, 0.85);
				vec4 white = vec4(0.259, 0.89, 0.608, 0.85);
				gl_FragColor = c == 1.0 ? black : white;
			}`,
